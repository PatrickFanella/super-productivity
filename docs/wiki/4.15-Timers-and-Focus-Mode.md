...
- **Standalone breaks** — From the Focus Mode preparation screen, the coffee-cup button starts a timed break for the currently tracked task without starting a work session. It uses the configured Pomodoro short-break duration. Break reminders use this same break screen when you choose **Pause & take a break**. Completing or skipping the break resumes the paused task and records its elapsed time as a break for the active work context.
