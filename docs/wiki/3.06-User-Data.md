...
Device-local support data is deliberately outside that snapshot. Examples include the sync conflict journal, operation-log transport metadata, the internal import-recovery slot, sync-provider private configuration stored in the separate `sup-sync` credential database (credentials and encryption key material), and issue-provider plugin fields declared as local-only secrets. Other issue-provider settings, including providers that have not adopted local-only credentials and legacy configurations that have not yet been migrated, can still contain API credentials or tokens.
