...
- **What it uses:** The bundled **CalDAV Events** plugin requests **VEVENT** components from one or more authenticated CalDAV calendars.
- **Calendar access:** Events appear in agenda-oriented views and can be created, updated, or deleted in a configured write calendar. A configurable sync window limits how far ahead events are queried.
- **Task mapping:** Linked tasks can synchronize title, notes, timed or all-day scheduling, and duration in either direction.
- **Automatic time blocking:** Scheduling a task can create a deterministic calendar event; rescheduling, completing, or deleting the task updates or removes that event.
- **Concurrent changes:** Updates and deletes use the server's current strong ETag. If another calendar client changes the event first, Super Productivity reports the conflict instead of overwriting that change.
- **Recurring events:** Recurring events are expanded for display, including exclusions and exceptions. Editing or deleting one occurrence is not supported; change that occurrence in the calendar application instead.
- **Authentication:** Username and password or app password using Basic authentication over HTTPS.
- **Credential storage:** After a CalDAV Events configuration is explicitly
  saved by a supporting app version, its password is stored only on that device
  and is excluded from sync, backup, and export data. Existing configurations
  continue using their legacy synchronized password until they are saved.
  Update other devices before saving, then enter a password or app password on
  each device. Changing the CalDAV server URL or username also requires the
  device-local password to be entered again.
- **Platform note:** Desktop and native mobile builds support the required CalDAV methods. The web app requires the CalDAV server or reverse proxy to allow the necessary CORS methods and headers.
