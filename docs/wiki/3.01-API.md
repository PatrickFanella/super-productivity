...
- `registerIssueProvider(definition)` — Registers an issue-provider plugin. A
  configuration field can declare `localOnly: true` to keep its string value in
  the host's device-local secret store instead of synchronized provider
  configuration. The host supplies local-only values only to asynchronous
  provider operations and header resolution; synchronous callbacks such as
  `getIssueLink` receive sanitized configuration. Provider configurations with
  the same plugin use separate local secret namespaces. Use
  `localOnlyScope: ['host', 'username']` for endpoint-bound credentials; the
  host requires re-entry when a scoped setting changes.
