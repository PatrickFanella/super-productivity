...
- You choose **Pause & take a break** — The reminder stops the current task and opens a timed Focus Mode break. The timer remains reset while that break is active.
...
## Starting And Finishing A Timed Break

Choose **Pause & take a break** on a break reminder to stop the current task and begin a timed Focus Mode break using your configured Pomodoro short-break duration (five minutes by default). The Focus Mode break screen shows the countdown and lets you pause, skip, or finish the break. When you finish or skip it, the previously paused task resumes if it is still available. The actual elapsed standalone break is added to the active work context’s break metrics.

You can also start the same standalone break from the Focus Mode preparation screen: while a task is tracked, select the coffee-cup button next to the start control. This does not replace an already running focus session.

If the reminder appears during a running Focus Mode work session, choosing **Pause & take a break** pauses that session before opening the standalone break. The paused task resumes after the break, but the interrupted Focus Mode work timer is not retained as a completed focus session.
...
Starting any Focus Mode break resets the break-reminder timer. The timer then resumes counting only after you return to task tracking, so scheduled and reminder-started breaks both count as a rest boundary.
...
- Break reminders use Focus Mode’s timed break lifecycle when you choose **Pause & take a break**; starting any Focus Mode break resets the reminder timer.
