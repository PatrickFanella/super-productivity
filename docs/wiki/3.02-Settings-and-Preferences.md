...
- **Tracking reminder** — When enabled, the reminder is quiet outside the configured
  work start/end window and during an enabled lunch break. The reminder supports
  overnight windows. Use **Pause until tomorrow** on a reminder banner to silence
  it for the current logical day on this device; it resumes automatically the next day.
